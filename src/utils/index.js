...
export function setParallax () {
    ...
    if ( parallax ) {
        let y = ( parallax.offsetTop - window.scrollY ) / 20 + 40;
...
export function stickyHeaderHandler ( stickyHeader, offsetTop, height ) {
    ...
    if ( stickyHeader.parentElement.classList.contains( 'sticky-wrapper' ) ) {
        top = window.scrollY + stickyHeader.parentElement.getBoundingClientRect().top + stickyHeader.parentElement.offsetHeight;
    } else {
        top = window.scrollY + stickyHeader.parentElement.getBoundingClientRect().top + stickyHeader.offsetHeight;
    }
...
export const countTo = function () {
    ...
    if ( items ) {
        for ( let i = 0; i < items.length; i++ ) {
            ...
            let pt = 0;
            let height = item.parentElement.parentElement.parentElement.getBoundingClientRect().top + window.scrollY;
            let flag = 0;

            document.addEventListener( "scroll", countToScrollHandler, false );

            function countToScrollHandler () {
                if ( pt <= time && height >= window.scrollY ) {
                    if ( 0 === flag ) {
